...
public:
  ...
  struct ClauseInfo {
    ...
    // The number of non-internal instructions in the clause.
    unsigned Size = 0;
      ...
      for (auto &MI : MBB) {
        ...
        if (CI.Size) {
          if (Type == HARDCLAUSE_INTERNAL) {
            ...
            ++CI.Length;
          } else if (Type == CI.Type && SII->shouldClusterMemOps(
                                            CI.BaseOps, BaseOps, CI.Size + 1)) {
        ...
        if (!CI.Size && Type < HARDCLAUSE_INTERNAL) {
          // Start a new clause.
          CI = ClauseInfo{Type, &MI, &MI, 1, 1, std::move(BaseOps)};
        }
      }
    }

    // Finish the last clause if any.
    Changed |= EmitClause(CI, SII);
